...
function showBanDialog(userId) {
    ...
    if (!banForm._banHandlerAttached) {
        banForm.addEventListener('submit', function(e) {
            ...
            // Исправлено: поддержка "Никогда" (кириллица) и "forever" (латиница)
            if (banPeriod === 'forever' || banPeriod === 'никогда') {
            ...
            // Формируем дату окончания в формате 2025-08-23 15:47:00
            const now = new Date();
            ...
            const endTime = `${endDateObj.getFullYear()}-${pad(endDateObj.getMonth() + 1)}-${pad(endDateObj.getDate())} ${pad(endDateObj.getHours())}:${pad(endDateObj.getMinutes())}:${pad(endDateObj.getSeconds())}`;
            fetch(`/api/ban-user/${userId}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
